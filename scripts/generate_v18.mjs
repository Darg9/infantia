...
children.push(new Paragraph({ children: [new TextRun({ text: "DOCUMENTO FUNDACIONAL V18", bold: true, size: 40, font: "Arial", color: DARK_BLUE })], alignment: AlignmentType.CENTER, spacing: { before: 0, after: 200 } }));
...
children.push(new Paragraph({ children: [new TextRun({ text: "2026-03-27", size: 24, font: "Arial", color: "777777" })], alignment: AlignmentType.CENTER, spacing: { before: 0, after: 1600 } }));
children.push(new Paragraph({ children: [new TextRun({ text: "Documento generado automaticamente por Claude Code — Infantia v0.8.0", size: 18, font: "Arial", color: "999999", italics: true })], alignment: AlignmentType.CENTER }));
...
children.push(twoColTable(
  ...
  [
    ...
    ["Busqueda", "Meilisearch (stub — pendiente activacion)"],
...
children.push(twoColTable(
  ...
  [
    ...
    ["src/lib/", "Utilidades compartidas (activity-url, category-utils, auth, db)"],
...
children.push(twoColTable(
  ...
  [
    ...
    ["8. Persistencia", "ScrapingStorage.saveActivity() — upsert por sourceUrl, preserva imageUrl existente"],
  ]
));
children.push(spacer());
...
children.push(twoColTable(
  ["Fuente", "Tipo"],
  [
    ["BibloRed (biblored.gov.co)", "Web — 153 actividades, paginacion automatica"],
    ...
    ["Banrep — Bucaramanga", "Sitemap XML filtrado por /bucaramanga/"],
    ["Banrep — Manizales", "Sitemap XML filtrado por /manizales/"],
    ["Banrep — Pereira", "Sitemap XML filtrado por /pereira/"],
    ["Banrep — Ibague", "Sitemap XML filtrado por /ibague/"],
    ["Banrep — Santa Marta", "Sitemap XML filtrado por /santa-marta/"],
  ]
));
children.push(spacer());
...
// ---- SECTION 7: FUNCIONALIDADES UI ----
children.push(sectionHeading("7. FUNCIONALIDADES DE INTERFAZ"));
children.push(subHeading("7.1 Paginas publicas"));
children.push(twoColTable(
  ["Ruta", "Descripcion"],
  [
    ["/", "Landing mejorada: stats reales, filtros rapidos por tipo, top 8 categorias, 8 actividades recientes"],
    ["/actividades", "Grid con filtros facetados + ordenamiento (5 criterios) + toggle Lista/Mapa + autocompletado"],
    ["/actividades/[uuid-slug]", "Detalle: hero imagen/gradiente, descripcion, fechas, precio, calificaciones, similares"],
...
children.push(spacer());
children.push(subHeading("7.2 Zona de usuario (requiere sesion)"));
children.push(twoColTable(
  ["Ruta", "Descripcion"],
  [
...
children.push(spacer());
children.push(subHeading("7.3 Panel admin (requiere rol ADMIN)"));
children.push(bodyParagraph("/admin — dashboard con links a fuentes y logs de scraping.", { bullet: true }));
children.push(bodyParagraph("/admin/scraping/sources — gestion de fuentes de scraping.", { bullet: true }));
children.push(bodyParagraph("/admin/scraping/logs — historial de ejecuciones.", { bullet: true }));
...
children.push(bodyParagraph("/admin/actividades/[id]/editar — formulario de edicion manual de actividades.", { bullet: true }));
children.push(bodyParagraph("API PATCH /api/admin/activities/[id] — actualizar status, title, price, imageUrl, etc.", { bullet: true }));
...
children.push(spacer());
children.push(subHeading("7.4 Componentes clave"));
children.push(twoColTable(
  ["Componente", "Descripcion"],
  [
    ["ActivityCard", "Tarjeta con strip h-20: imagen real o gradiente por categoria + emoji"],
...
// ---- SECTION 8: SEO Y CANONICALIZACION ----
children.push(sectionHeading("8. SEO Y CANONICALIZACION DE URLs"));
children.push(subHeading("8.1 URLs canonicas"));
...
children.push(spacer());
children.push(subHeading("8.2 Metadata y structured data"));
...
// ---- SECTION 9: IMAGENES ----
children.push(sectionHeading("9. GESTION DE IMAGENES"));
children.push(twoColTable(
  ["Estrategia", "Detalle"],
  [
    ...
    ["BibloRed", "153 actividades sin og:image — muestran gradiente segun categoria"],
    ["Detalle sin imagen", "Hero h-44 con gradiente + emoji centrado + badges backdrop-blur"],
  ]
));
children.push(spacer());

// ---- SECTION 10: AUTENTICACION Y USUARIOS ----
children.push(sectionHeading("10. AUTENTICACION Y GESTION DE USUARIOS"));
children.push(subHeading("10.1 Flujo de autenticacion"));
...
children.push(spacer());
children.push(subHeading("10.2 Cumplimiento legal (Ley 1581 Colombia)"));
...
children.push(bodyParagraph("Consentimiento parental explícito en creacion de perfiles de hijos.", { bullet: true }));
children.push(bodyParagraph("SIC_RNBD_Registro.md — guia de registro ante la SIC.", { bullet: true }));
children.push(spacer());

// ---- SECTION 11: NOTIFICACIONES POR EMAIL ----
children.push(sectionHeading("11. NOTIFICACIONES POR EMAIL"));
children.push(bodyParagraph("Proveedor: Resend con react-email para templates HTML.", { bullet: true }));
...
children.push(bodyParagraph("Filtros: email habilitado, frecuencia daily/weekly, categoria newActivities, edad hijos.", { bullet: true }));
children.push(bodyParagraph("Cron Vercel: 9am UTC diario (vercel.json).", { bullet: true }));
children.push(spacer());

// ---- SECTION 11b: WEB PUSH ----
children.push(sectionHeading("11b. NOTIFICACIONES PUSH (WEB PUSH API)"));
...
children.push(bodyParagraph("API DELETE /api/push/subscribe — elimina suscripcion del usuario autenticado.", { bullet: true }));
children.push(bodyParagraph("src/lib/push.ts — sendPushNotification() y sendPushToMany() con web-push npm.", { bullet: true }));
children.push(bodyParagraph("PushButton component — estados: loading | unsupported | denied | subscribed | unsubscribed.", { bullet: true }));
children.push(bodyParagraph("Cron send-notifications envia push a todas las suscripciones activas + limpia endpoints expirados.", { bullet: true }));
children.push(bodyParagraph("Modelo PushSubscription en Prisma — tabla push_subscriptions en Supabase.", { bullet: true }));
children.push(spacer());

// ---- SECTION 11c: PAGINA DE PROVEEDOR ----
children.push(sectionHeading("11c. PAGINA DE PROVEEDOR"));
children.push(bodyParagraph("Ruta /proveedores/[slug] — pagina publica con perfil del proveedor y todas sus actividades.", { bullet: true }));
children.push(bodyParagraph("Campo slug unico en modelo Provider — generado automaticamente al crear proveedor.", { bullet: true }));
children.push(bodyParagraph("Script backfill-provider-slugs.ts — genera slugs para proveedores existentes.", { bullet: true }));
children.push(bodyParagraph("Links desde ActivityCard y detalle de actividad → pagina del proveedor.", { bullet: true }));
children.push(bodyParagraph("Muestra: nombre, tipo, website, contador de actividades, grid de actividades activas.", { bullet: true }));
children.push(spacer());

// ---- SECTION 11d: FEATURES v0.8.0 ----
children.push(sectionHeading("11d. FEATURES v0.8.0 — BUSQUEDA, MAPA Y METRICAS"));
children.push(subHeading("Autocompletado de busqueda"));
children.push(bodyParagraph("API GET /api/activities/suggestions?q= — hasta 6 sugerencias ordenadas por confianza.", { bullet: true }));
children.push(bodyParagraph("Dropdown con highlight del texto buscado (<mark>), debounce 200ms.", { bullet: true }));
children.push(bodyParagraph("Navegacion teclado: ArrowUp/Down, Enter va a actividad, Escape cierra.", { bullet: true }));
children.push(bodyParagraph("Click en texto aplica como filtro; flecha va directamente a la actividad.", { bullet: true }));
children.push(spacer());
children.push(subHeading("Ordenamiento en /actividades"));
children.push(bodyParagraph("5 criterios: Relevancia (default) | Proximas | Recien agregadas | Precio menor | Precio mayor.", { bullet: true }));
children.push(bodyParagraph("buildOrderBy() con nulls:last para precio y fecha — actividades sin dato van al final.", { bullet: true }));
children.push(bodyParagraph("El ordenamiento se preserva al cambiar cualquier otro filtro.", { bullet: true }));
children.push(spacer());
children.push(subHeading("Pagina de inicio mejorada"));
children.push(bodyParagraph("Stats reales en hero: actividades, ciudades, categorias, fuentes (queries en paralelo).", { bullet: true }));
children.push(bodyParagraph("Filtros rapidos por tipo: Talleres / Campamentos / Recurrentes / Eventos con count real.", { bullet: true }));
children.push(bodyParagraph("Categorias destacadas: top 8 con gradiente de color + emoji + count.", { bullet: true }));
children.push(bodyParagraph("8 actividades recientes (sortBy newest), CTA final con banner indigo.", { bullet: true }));
children.push(spacer());
children.push(subHeading("Badge 'Nuevo' en tarjetas"));
children.push(bodyParagraph("Actividades agregadas en los ultimos 7 dias muestran badge rojo '🆕 Nuevo'.", { bullet: true }));
children.push(bodyParagraph("No aparece en actividades EXPIRED. Desaparece automaticamente al pasar 7 dias.", { bullet: true }));
children.push(spacer());
children.push(subHeading("Mapa de actividades con pines Leaflet"));
children.push(bodyParagraph("Toggle Lista/Mapa en /actividades — alternativa al grid de tarjetas.", { bullet: true }));
children.push(bodyParagraph("API GET /api/activities/map — hasta 500 actividades ACTIVE con coords, respeta todos los filtros.", { bullet: true }));
children.push(bodyParagraph("MapInner.tsx — Leaflet real (dynamic, ssr:false), pines indigo custom DivIcon.", { bullet: true }));
children.push(bodyParagraph("Popup: titulo + categoria + ubicacion + precio + enlace 'Ver actividad →'.", { bullet: true }));
children.push(bodyParagraph("fitBounds automatico para mostrar todos los pines del resultado actual.", { bullet: true }));
children.push(bodyParagraph("Geocoding via Nominatim — script geocode-activities.ts backfilleo 230 actividades existentes.", { bullet: true }));
children.push(spacer());
children.push(subHeading("Panel de metricas admin (/admin/metricas)"));
children.push(bodyParagraph("ActivityView: registro anonimo de cada visita (activityId, viewedAt).", { bullet: true }));
children.push(bodyParagraph("SearchLog: registro de busquedas con query normalizado y resultCount.", { bullet: true }));
children.push(bodyParagraph("Tracking automatico: ActivityHistoryTracker llama /view; Filters.tsx registra busquedas.", { bullet: true }));
children.push(bodyParagraph("Panel: resumen general | vistas hoy/7d/30d | top 10 mas vistas | top 20 busquedas frecuentes.", { bullet: true }));
children.push(bodyParagraph("Busquedas sin resultados — oportunidades de contenido faltante.", { bullet: true }));
children.push(bodyParagraph("Distribucion por tipo (barras CSS) + top 5 proveedores por volumen.", { bullet: true }));
children.push(spacer());

// ---- SECTION 12: ESTADO ACTUAL ----
children.push(sectionHeading("12. ESTADO ACTUAL — v0.8.0 (2026-03-27)"));
children.push(twoColTable(
  ["Metrica", "Valor"],
  [
    ["Actividades en BD", "~245 (167 BibloRed + 21 bogota.gov.co + 12 Instagram + ~45 CRD)"],
    ["Actividades ACTIVE", "~30 (resto EXPIRED — eventos pasados, normal)"],
    ["Actividades con imagen real", "77 de 245 (31%) — og:image extraida por backfill"],
    ["Tests", "695 tests en 46 archivos — todos verdes"],
    ...
    ["Fuentes configuradas", "14 (4 Bogota + 10 Banrep multi-ciudad)"],
    ["Gemini quota", "20 RPD free tier — renovacion medianoche UTC"],
    ["Geocoding", "Nominatim (OpenStreetMap) — 10 locations, 230+ actividades geocodificadas"],
  ]
));
children.push(spacer());
children.push(subHeading("12.1 Historial de versiones"));
...
children.push(bodyParagraph("Activar Meilisearch — busqueda full-text con relevancia, facetas y typo-tolerance.", { bullet: true }));
...
children.push(bodyParagraph("Modelo de monetizacion definido tras analisis de uso real.", { bullet: true }));
children.push(spacer());

// ---- SECTION 14: PENDIENTE ----
children.push(sectionHeading("14. PENDIENTE INMEDIATO"));
children.push(twoColTable(
  ["Item", "Detalle"],
  [
    ["Tablas BD metricas", "Ejecutar SQL en Supabase: activity_views + search_logs + indices (ver commit d36cffa)"],
    ["Ingesta con Gemini", "npx tsx scripts/ingest-sources.ts --queue + npx tsx scripts/run-worker.ts (con quota fresca)"],
    ["Banrep pendiente", "URLs de Bogota sin procesar — prioridad para llegar a 500+ actividades"],
    ["Mapa detalle actividad", "Mini-mapa embed en /actividades/[id] mostrando ubicacion exacta"],
    ["Vista calendario", "/actividades?view=calendar — actividades por fecha en calendario mensual"],
    ["Activar Meilisearch", "Indice de busqueda full-text — configurar en Vercel cuando haya 500+ actividades"],
    ["Segunda ciudad", "Medellin o Cali con datos completos"],
  ]
));
children.push(spacer());

// ---- SECTION 15: SCRIPTS UTILES ----
children.push(sectionHeading("15. SCRIPTS Y COMANDOS UTILES"));
children.push(twoColTable(
  ["Comando", "Descripcion"],
  [
    ...
    ["npx tsx scripts/geocode-activities.ts", "Backfill geocoding para actividades sin ubicacion"],
    ["node scripts/generate_v18.mjs", "Generar este documento Fundacional V18"],
  ]
));
children.push(spacer());
...
// ---- FINAL NOTE ----
children.push(new Paragraph({ children: [new PageBreak()] }));
children.push(new Paragraph({ children: [new TextRun({ text: "Infantia — Documento Fundacional V18", size: 18, font: "Arial", color: "999999", italics: true })], alignment: AlignmentType.CENTER, spacing: { before: 400, after: 80 } }));
children.push(new Paragraph({ children: [new TextRun({ text: "Generado por Claude Code el 2026-03-27 | Version del software: v0.8.0", size: 16, font: "Arial", color: "BBBBBB" })], alignment: AlignmentType.CENTER }));
      ...
      headers: {
        default: new Header({
          children: [
            new Paragraph({
              children: [
                new TextRun({ text: "INFANTIA — DOCUMENTO FUNDACIONAL V18", size: 16, font: "Arial", color: "999999" }),
              ],
...
const OUTPUT = "C:/Users/denys/OneDrive/Documents/DayJul/Denys/Infantia/Infantia_Claude/Infantia_Documento_Fundacional_V18.docx";
