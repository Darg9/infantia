...
children.push(
  new Paragraph({
    children: [
      new TextRun({
        text: "DOCUMENTO FUNDACIONAL V08",
        bold: true,
        size: 40,
        font: "Arial",
        color: DARK_BLUE,
...
children.push(
  new Paragraph({
    children: [
      new TextRun({
        text: "Documento generado automáticamente por Claude Code — Infantia v0.8",
        size: 18,
        font: "Arial",
        color: "999999",
        italics: true,
...
children.push(spacer());
children.push(new Paragraph({ children: [new TextRun({ text: "Pipeline de Instagram (Session 9):", bold: true, size: 20, font: "Arial", color: DARK_BLUE })], spacing: { before: 80, after: 60 } }));
for (const c of [
  "PlaywrightExtractor: Chromium headless, desktop UA, viewport 1280x800",
  "Sesión persistente: Login manual one-time, cookies guardadas en ig-session.json",
  "Extracción: Perfil → bio + grid de posts → navegación individual a cada post",
  "Datos por post: caption, imágenes, timestamp, likes",
  "Anti-detección: delays aleatorios 2-5s, locale es-CO, timezone America/Bogota",
  "NLP: Prompt Gemini específico para Instagram (hashtags como categorías, emojis como indicadores)",
...
children.push(
  threeColTable(
    ...
    [
      ...
      ["Alcaldía de Bogotá (bogota.gov.co)", "21 (10.5%)", "20 / 21 (95%)"],
      ["FCE Colombia (Instagram @fcecolombia)", "10 (5%)", "8 / 10 (80%)"],
      ["Qué hay pa hacer (Instagram @quehaypahacerenbogota)", "2 (1%)", "2 / 2 (100%)"],
    ],
    [50, 25, 25]
  )
);
...
children.push(spacer());
children.push(new Paragraph({ children: [new TextRun({ text: "Cobertura de tests (v0.8):", bold: true, size: 20, font: "Arial", color: DARK_BLUE })], spacing: { before: 80, after: 60 } }));
children.push(spacer(0.5));
children.push(
  threeColTable(
    ["Módulo / Archivo", "Cobertura líneas", "Tests"],
    [
      ["category-utils.ts", "100%", "14 tests"],
      ["api-response.ts", "100%", "9 tests"],
      ["claude.analyzer.ts", "100%", "11 tests"],
      ["gemini.analyzer.ts", "98.8%", "23 tests"],
      ["pipeline.ts", "100%", "19 tests"],
      ["cheerio.extractor.ts", "91.4%", "15 tests"],
      ["playwright-extractor.test.ts", "100%", "8 tests"],
      ["activities.service.ts", "100% funciones", "—"],
      ["TOTAL PROYECTO", "—", "212 tests"],
    ],
    [50, 25, 25]
  )
);
...
children.push(
  threeColTable(
    ...
    [
      ...
      ["Fase 1 MVP", "Scraping Instagram", "✅ Completado"],
...
children.push(
  threeColTable(
    ...
    [
      ...
      ["Qué hay pa hacer (@quehaypahacerenbogota)", "Instagram (Playwright)", "✅ 2 actividades"],
...
children.push(sectionHeading("16. HISTORIAL DE VERSIONES"));
children.push(spacer(0.5));
children.push(
  threeColTable(
    ...
    [
      ...
      ["V07", "2026-03-16", "Claude Code — Cobertura de tests 32% → 95.8% (193 tests), diagnóstico de fuentes verificado (BibloRed 167 + Alcaldía 21), script check-sources.ts, versión estable"],
      ["V08", "2026-03-16", "Claude Code — Instagram como tercera fuente (Playwright + sesión), 12 actividades IG en Supabase, 200 actividades totales, 212 tests, prompt Gemini para IG"],
    ],
    [10, 18, 72]
  )
);
...
children.push(
  new Paragraph({
    children: [
      new TextRun({
        text: "Documento generado automáticamente por Claude Code — Infantia v0.8 — 2026-03-16",
        size: 16,
        font: "Arial",
        color: "999999",
        italics: true,
...
const outputPath = process.env.OUTPUT_PATH || String.raw`C:\Users\denys\OneDrive\Documents\DayJul\Denys\Infantia\Infantia_Claude\Infantia_Documento_Fundacional_V08.docx`;
