...
children.push(new Paragraph({
  children: [new TextRun({ text: "DOCUMENTO FUNDACIONAL V21", bold: true, size: 40, font: "Arial", color: DARK_BLUE })],
...
children.push(new Paragraph({
  children: [new TextRun({ text: "Sesiones S25-S26 — Seguridad, Observabilidad y Scraping Inteligente", size: 20, font: "Arial", color: "999999", italics: true })],
  alignment: AlignmentType.CENTER, spacing: { before: 0, after: 1200 },
}));
children.push(new Paragraph({
  children: [new TextRun({ text: "Documento generado automaticamente por Claude Code — Infantia v0.9.0 (commits 50c7f97 → b1e764a)", size: 18, font: "Arial", color: "BBBBBB", italics: true })],
  alignment: AlignmentType.CENTER,
}));
...
children.push(bodyParagraph("Formato de linea: 2026-03-31T22:00:00Z INFO  [ctx] mensaje {\"meta\":\"json\"}", { bullet: true }));
...
children.push(bodyParagraph("Listo para UptimeRobot / BetterUptime — URL: https://infantia-activities.vercel.app/api/health", { bullet: true }));
children.push(spacer());
...
children.push(bodyParagraph("Resultado al 2026-03-31: 29/29 locations en BD con coordenadas reales (lat/lng != 0).", { bullet: true }));
children.push(spacer());
...
children.push(bodyParagraph("UTM tracking en todos los links del digest: ?utm_source=infantia&utm_medium=email&utm_campaign=digest_{daily|weekly}", { bullet: true }));
...
children.push(sectionHeading("14. ESTADO ACTUAL — v0.9.0 (2026-03-31)"));
children.push(twoColTable(
  ["Metrica", "Valor"],
  [
    ["Actividades en BD", "~277 (260 base + 17 nuevas S26: Cinemateca 13, JBB 3, Banrep Cartagena 1)"],
    ["Estado actividades", "Mayoria EXPIRED (cron expira por fecha < hoy). Pendiente: Banrep ingest completo."],
    ["Locations geocodificadas", "29/29 con coordenadas reales (lat/lng != 0)"],
    ["Tests", "783 tests en 51 archivos — todos verdes"],
    ["Cobertura", "91.76% stmts / 86.98% branches / 89.73% funcs (umbral 85% superado)"],
    ["TypeScript", "0 errores (tsc --noEmit)"],
    ["npm audit", "0 vulnerabilidades (era 15 antes de S25)"],
    ...
    ["Sentry", "Integrado — pendiente crear cuenta sentry.io y agregar SENTRY_DSN en Vercel"],
    ["UptimeRobot", "Pendiente configurar — URL: https://infantia-activities.vercel.app/api/health"],
  ]
));
children.push(spacer());
...
children.push(twoColTable(
  ...
  [
    ...
    ["Tests totales", "783 en 51 archivos"],
...
children.push(spacer());
children.push(subHeading("Tests nuevos en V21 (S25)"));
children.push(bodyParagraph("geocoding.test.ts (19 tests): venue-dictionary hit, Nominatim success/params/user-agent, empty→city fallback (5 rutas), HTTP errors, network errors. Cobertura geocoding.ts: 8% → 95%.", { bullet: true }));
children.push(bodyParagraph("push.test.ts (16 tests): sendPushNotification (success, 410 expired, 404 expired, 500 non-fatal, 400, network error), sendPushToMany (empty, all ok, mixed expiry, all expired). Cobertura push.ts: 0% → 94%.", { bullet: true }));
children.push(bodyParagraph("Patron clave para throttle sin fake timers: vi.spyOn(Date, 'now').mockImplementation(() => { fakeNow += 5000; return fakeNow; }) — evita timeout por setTimeout del rate limiter.", { bullet: true }));
children.push(spacer());

// ── SECTION 16: ROADMAP ──────────────────────────────────────────────────────
children.push(sectionHeading("16. ROADMAP"));
children.push(subHeading("Corto plazo — v0.9.x (pendiente de activacion)"));
children.push(twoColTable(
  ["Item", "Accion requerida"],
  [
    ["Activar Sentry", "Crear cuenta sentry.io → New Project → Next.js → SENTRY_DSN en Vercel Dashboard"],
    ["Activar UptimeRobot", "Monitor URL https://infantia-activities.vercel.app/api/health — gratuito, alertas email"],
    ["Activar proxy IPRoyal", "Comprar credito $7/GB → PLAYWRIGHT_PROXY_SERVER/USER/PASS en Vercel env vars"],
    ["Banrep Cartagena", "Correr ingest cuando Gemini resetee (19:00 COL): npx tsx scripts/ingest-sources.ts --source=banrep --save-db"],
  ]
));
children.push(spacer());
...
children.push(twoColTable(
  ...
  [
    ...
    ["npm test", "Correr todos los tests una vez (783 tests)"],
    ["npm run test:coverage", "Tests + reporte de cobertura (threshold 85%)"],
    ["npx tsx scripts/ingest-sources.ts --list", "Ver inventario de fuentes por canal — NUEVO V21"],
    ["npx tsx scripts/ingest-sources.ts --save-db", "Ingest completo a BD (todas las fuentes)"],
    ["npx tsx scripts/ingest-sources.ts --source=banrep --save-db", "Solo Banrep — ahorra cuota Gemini — NUEVO V21"],
    ["npx tsx scripts/ingest-sources.ts --channel=web --save-db", "Solo fuentes web — NUEVO V21"],
    ...
    ["node scripts/generate_v21.mjs", "Generar este Documento Fundacional V21"],
  ]
));
children.push(spacer());
...
children.push(new Paragraph({ children: [new PageBreak()] }));
children.push(new Paragraph({
  children: [new TextRun({ text: "Infantia — Documento Fundacional V21", size: 18, font: "Arial", color: "999999", italics: true })],
  alignment: AlignmentType.CENTER, spacing: { before: 400, after: 80 },
}));
children.push(new Paragraph({
  children: [new TextRun({ text: "Generado por Claude Code el 2026-03-31 | Software: v0.9.0 (50c7f97 → b1e764a) | 783 tests | 51 archivos | 0 vulnerabilidades npm", size: 16, font: "Arial", color: "BBBBBB" })],
  alignment: AlignmentType.CENTER,
}));
  ...
  sections: [
    {
      headers: {
        default: new Header({
          children: [
            new Paragraph({
              children: [new TextRun({ text: "INFANTIA — DOCUMENTO FUNDACIONAL V21", size: 16, font: "Arial", color: "999999" })],
...
const OUTPUT_PATH = "C:\\Users\\denys\\OneDrive\\Documents\\DayJul\\Denys\\Infantia\\Infantia_Claude\\Infantia_Documento_Fundacional_V21.docx";
