...
const PLAZOS: Record<string, string> = {
  'Solicitud de remoción de contenido':                                         'máximo 5 días hábiles',
  'Ejercer derechos de datos personales (acceso, rectificación, cancelación)':  'máximo 10 días hábiles',
};
...
export async function POST(req: NextRequest) {
  ...
  const { motivo, nombre, email, mensaje, actividadUrl } = body as Record<string, string>;
    ...
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#111">
      ...
      <table style="width:100%;border-collapse:collapse;margin:16px 0;font-size:14px">
        <tr style="background:#f9f9f9">
          ...
          <td style="padding:8px 12px;border:1px solid #eee">${motivo}</td>
        </tr>
        <tr>
  ...
  const plazo    = PLAZOS[motivo] || 'a la brevedad posible';
  const htmlUser = `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#111">
      <h2 style="color:#ea580c">Recibimos tu mensaje</h2>
      <p>Hola <strong>${nombreClean}</strong>,</p>
      <p>Hemos recibido tu solicitud de contacto con el motivo <strong>${motivo}</strong>. Te responderemos <strong>${plazo}</strong>.</p>
  ...
  try {
    ...
    const [resEquipo, resUser] = await Promise.all([
      resend.emails.send({
        ...
        to:       CONTACT_EMAIL,
        replyTo:  emailClean,
        subject:  `[Contacto] ${motivo} — ${nombreClean}`,
        html:     htmlEquipo,
      }),
      resend.emails.send({
        from:    FROM_EMAIL,
    ...
    if (resEquipo.error) {
      log.error('Error enviando email equipo', { error: resEquipo.error, motivo });
      return NextResponse.json({ error: 'Error al enviar. Intenta de nuevo o escríbenos a info@habitaplan.com' }, { status: 500 });
    }

    log.info('Contacto recibido', { motivo, email: emailClean });
